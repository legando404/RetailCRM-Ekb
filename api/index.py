...
site = os.getenv('site')#= 'ekaterinburg-novers-ru'
...
async def get_mail(username, password, imap_server):
    ...
    with MailBox(imap_server).login(username, password, initial_folder='Novers Екатеринбург') as mailbox:
        print('fetching...')
        exists = mailbox.folder.exists('Novers Екатеринбург/INBOX|Екб')
        if not exists:
            mailbox.folder.create('Novers Екатеринбург/INBOX|Екб')
       
        # Берём все письма, вне зависимости от прочитанности
        for msg in mailbox.fetch():
            mailbox.move(msg.uid, 'Novers Екатеринбург/INBOX|Екб') 
